...
interface ApiServiceEntry {
  ...
  hint: string;
  path: string;
  /** When set, this service follows ResourceId (show the three call shapes). */
  resourceCalls?: {
    page: string;
    all: string;
    byId: string;
  };
  /** Non-ResourceId return line (user / file / mode-config). */
  returns?: string;
  model: string;
  code: string;
}
...
/**
 * Reference catalog of HTTP services in `@aies/aies-core`.
 * Docs only — no live API calls from this page.
 */
@Component({
  ...
  template: `
    <div class="pg-page-enter flex flex-col gap-10">
      <app-page-header
        eyebrow="Foundation"
        title="SDK API"
        description="Ready-made HTTP services and the ResourceId convention for list GETs. Paths live in @aies/aies-core; shapes in @aies/aies-models. Open Show code for usage — this page does not call the API."
      />
      ...
      >
        <p class="m-0 text-body-sm text-neutral-600 dark:text-neutral-400">
          After setup, inject domain services below — or use ApiClient helpers for
          custom routes.
        </p>
      </app-demo-section>
      ...
      <app-demo-section
        title="ResourceId"
        hint="Every list-style GET uses the same id segment. Built-in services and custom ApiClient calls share this rule."
      >
        <div class="flex flex-col gap-5">
          <div class="overflow-x-auto">
            <table class="w-full min-w-[36rem] border-collapse text-left text-body-sm">
              <thead>
                <tr class="border-b border-border text-caption uppercase tracking-wide text-neutral-500 dark:border-white/10">
              ...
              <tbody>
                @for (row of resourceIdRows; track row.id) {
                  <tr class="border-b border-border/70 last:border-0 dark:border-white/10">
                    <td class="py-3 pr-4 align-top">
                      <code class="font-mono text-ink dark:text-white">{{
                        row.id
                      }}</code>
                    </td>
                    <td class="py-3 pr-4 align-top text-neutral-600 dark:text-neutral-400">
                      {{ row.meaning }}
                    </td>
                    <td class="py-3 pr-4 align-top">
                      <code class="font-mono text-caption text-ink dark:text-white">{{
                        row.path
                      }}</code>
                    </td>
                    <td class="py-3 pr-4 align-top">
          ...
          <p class="m-0 text-caption text-neutral-500 dark:text-neutral-400">
            ...
            <code class="font-mono">order</code>. Page size defaults to the
            backend
            <code class="font-mono">api.paginate.*.pageSize</code> config when
            omitted. Bind
            <code class="font-mono">res.pagination</code> to
            <code class="font-mono">aies-pagination</code>.
          </p>
        </div>
      </app-demo-section>
      ...
      @for (entry of services; track entry.id) {
        <app-demo-section
          ...
          [code]="entry.code"
        >
          <dl class="m-0 grid gap-3 text-body-sm sm:grid-cols-[7rem_1fr] sm:gap-x-4 sm:gap-y-3">
            <dt class="m-0 text-caption font-medium uppercase tracking-wide text-neutral-500">
              Path
            </dt>
            ...
            @if (entry.resourceCalls; as calls) {
              ...
              <dd class="m-0 flex flex-col gap-1.5 font-mono text-caption text-ink dark:text-white">
            ...
            <dd class="m-0 text-neutral-600 dark:text-neutral-400">
              <span class="font-mono text-ink dark:text-white">{{
                entry.model
              }}</span>
              <span class="text-caption"> — Foundation → Models</span>
            </dd>
          </dl>
        </app-demo-section>
      }
...
export class ApiPage {
  ...
  protected readonly services: ApiServiceEntry[] = [
    {
      id: 'country',
