...
import type { AfricaniesHttpToastHandler } from '@africanies/africanies-core';
...
export class ToastService implements AfricaniesHttpToastHandler {
   ...
   * Called from {@link provideAfricaniesToasts}.
   */
  ensureHost(): void {
    ...
    ref.hostElement.classList.add('africanies-toast-overlay');
  ...
  show(options: ToastShowOptions): string {
    ...
    const id = `africanies-toast-${++this.seq}`;
