...
@Component({
  ...
  template: `
    <africanies-overlay-frame>
      <div africaniesOverlayHeader>Header chrome</div>
      <p>Body copy</p>
      <div africaniesOverlayFooter>Footer actions</div>
    </africanies-overlay-frame>
  `,
})
...
describe('OverlayFrameComponent', () => {
  ...
  it('keeps header and footer outside the scroll region', () => {
    ...
    const frame = root.querySelector('africanies-overlay-frame') as HTMLElement;
    ...
    expect(bodySlot.className).toContain('africanies-overlay-scroll');
