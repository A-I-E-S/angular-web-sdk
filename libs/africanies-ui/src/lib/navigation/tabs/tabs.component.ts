 ...
 * `<ng-template africaniesTabDef="id">`.
 *
 * @example
 * ```html
 * <!-- URL-driven (child routes) -->
 * <africanies-tabs [items]="[
 ...
 */
@Component({
  selector: 'africanies-tabs',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [AfricaniesIconComponent, NgTemplateOutlet],
  template: `
        ...
        @for (item of items(); track item.id) {
          @if (item.routerLink !== null && item.routerLink !== undefined) {
            <a
              ...
              [attr.id]="'africanies-tab-' + item.id"
            ...
            <button
              ...
              [attr.id]="'africanies-tab-' + item.id"
              [attr.aria-controls]="
                hasPanels() ? 'africanies-tab-panel-' + item.id : null
              "
      ...
      @if (hasPanels()) {
        @if (activePanel(); as panel) {
          <div
            ...
            [attr.id]="'africanies-tab-panel-' + activeId()"
            [attr.aria-labelledby]="'africanies-tab-' + activeId()"
          >
            <ng-container
...
export class TabsComponent {
  ...
  private readonly panelMap = computed(() => {
    ...
    for (const def of this.tabDefs()) {
      map.set(def.africaniesTabDef(), def.template);
    }
    return map;
  });
  ...
  protected onTablistKeydown(event: KeyboardEvent): void {
    ...
    if (nextEl.tagName === 'BUTTON') {
      const id = nextEl.id.replace(/^africanies-tab-/, '');
