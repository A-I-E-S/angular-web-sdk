...
export function isNestedChildRoute(
  url: string,
  navItems: AfricaniesSideNavItem[],
): boolean {
...
export function buildBreadcrumbsFromSideNav(
  ...
  const crumbs: AfricaniesNavItem[] = [
...
function resolveHomeRouterLink(navItems: AfricaniesSideNavItem[]): string {
...
function isAppRoot(path: string, navItems: AfricaniesSideNavItem[]): boolean {
  if (path === '/') {
    return true;
  }
...
function findSideNavMatch(path: string, navItems: AfricaniesSideNavItem[]) {
  let best:
    | {
        group?: Pick<AfricaniesSideNavItem, 'id' | 'label' | 'icon'>;
...
function finalizeLeaf(crumbs: AfricaniesNavItem[], path: string): AfricaniesNavItem[] {
  if (crumbs.length < 2) {
    return crumbs;
  }
