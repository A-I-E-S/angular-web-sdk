 ...
 * {@link ToastService} is registered via `provideAfricaniesToasts()`.
 *
 * @example
 * ```html
 * <africanies-copy [value]="row.reference" />
 * <africanies-copy [value]="snippet" label="Copy code" />
 * <africanies-copy [value]="ref" ariaLabel="Copy reference" [announce]="true" />
 * ```
 */
@Component({
  selector: 'africanies-copy',
  standalone: true,
  ...
  template: `
    <button
      ...
      (click)="onCopy()"
    >
      <africanies-icon [name]="showCopied() ? 'check' : 'copy'" [size]="iconSize()" />
      @if (label(); as text) {
...
export class CopyButtonComponent {
  ...
  /** Extra classes on the inner `africanies-button` (e.g. inverted code headers). */
