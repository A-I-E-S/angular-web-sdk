 ...
 */
@Component({
  selector: 'africanies-camera-capture-dialog',
  standalone: true,
    ...
    >
      <div class="flex items-start justify-between gap-3">
        ...
        <button
          ...
          (click)="switchFacing()"
        >
          <africanies-icon name="refresh" [size]="16" />
        </button>
      </div>
      ...
      <div class="flex flex-wrap justify-end gap-2">
        <button africanies-button type="button" variant="ghost" (click)="cancel()">
          Cancel
        </button>
        <button
          africanies-button
...
export class CameraCaptureDialogComponent {
  ...
  protected readonly titleId = `africanies-camera-title-${Math.random().toString(36).slice(2, 10)}`;
