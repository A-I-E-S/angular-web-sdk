...
export interface SelectOption<T = string> {
  ...
  disabled?: boolean;
  /**
   * Leading icon slot for this option (`africanies-icon` name).
  ...
  prefixText?: string;
  /**
   * Trailing icon slot for this option (`africanies-icon` name).
   * Omit when the row is label-only.
   */
  suffix?: IconName;
}
 ...
 *
 * - `md` — default field height (`h-10`), matches md `africanies-button`
 * - `sm` — compact (`h-8`), matches sm `africanies-button` (pagination size picker)
 */
