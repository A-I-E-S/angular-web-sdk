    ...
    >
      @if (shellPrefix(); as prefixIcon) {
        <span [class]="affixClass" data-slot="prefix">
          <africanies-icon [name]="prefixIcon" [size]="16" class="shrink-0" />
        </span>
      ...
      >
        @if (multiple() && selectedList().length) {
          @for (chip of selectedList(); track trackOption(chip)) {
            <span
              class="inline-flex items-center gap-1 rounded-md border border-neutral-300 bg-background-welcome dark:border-white/20 dark:bg-white/10 text-body-sm text-ink dark:text-white px-2 py-0.5"
            >
              ...
              @if (chip.suffix; as suffixIcon) {
                <africanies-icon [name]="suffixIcon" [size]="14" class="shrink-0" />
              }
              <button
                type="button"
              ...
              >
                <africanies-icon name="close-small" [size]="12" />
              </button>
            </span>
          }
        }
      ...
      @if (shellSuffix(); as suffixIcon) {
        <span [class]="affixClass" data-slot="suffix">
          <africanies-icon [name]="suffixIcon" [size]="16" class="shrink-0" />
        </span>
      }
      <span
        [class]="affixClass"
      ...
      >
        ...
        @if (loading()) {
          ...
          @if (create(); as createCfg) {
            ...
            >
              <africanies-icon name="file-add" [size]="16" />
              {{ createCfg.label }}
            </button>
          }
          ...
          >
            @if (multiple()) {
              <span [class]="multiCheckClass(opt)">
                @if (isSelected(opt)) {
                  <africanies-icon name="check" [size]="12" />
                }
              </span>
            }
...
export class SelectComponent<T = string> implements ControlValueAccessor {
  ...
  protected readonly controlId = `africanies-select-${++nextSelectId}`;
   ...
   * sm `africanies-button` (pagination size picker).
   */
  ...
  protected openCreateModal(): void {
    ...
    if (!this.modal) {
      // WHY console error: create requires provideAfricaniesUiOverlays() / ModalService.
      console.error(
        '[africanies-select] create requires ModalService — call provideAfricaniesUiOverlays() at bootstrap.',
      );
      return;
    }
