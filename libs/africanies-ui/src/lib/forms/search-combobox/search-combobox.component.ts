 ...
 */
@Component({
  selector: 'africanies-search-combobox',
  standalone: true,
  ...
  template: `
    ...
    >
      ...
      <span [class]="affixClass" data-slot="suffix">
        @if (loading()) {
          <africanies-icon name="spinner" [size]="16" class="animate-spin" />
        } @else if (selectedMark()) {
          <africanies-tooltip [text]="markTooltip()" placement="top">
            <span
            ...
            >
              <africanies-icon name="star" [size]="14" class="text-[#D4AF37]" />
            </span>
          </africanies-tooltip>
        } @else {
...
export class SearchComboboxComponent<T> implements ControlValueAccessor {
  ...
  protected readonly controlId = `africanies-search-combobox-${++nextSearchComboboxId}`;
