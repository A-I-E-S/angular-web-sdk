 ...
 * @example
 * ```html
 * <ng-template aiesHeaderCellDef="checkbox">
 *   <aies-checkbox label="" [value]="allChecked()" (valueChange)="toggleAll($event)" />
 * </ng-template>
 * ```
 */
@Directive({
  selector: '[aiesHeaderCellDef]',
  standalone: true,
})
export class HeaderCellDefDirective {
   ...
   */
  readonly aiesHeaderCellDef = input.required<string>();
}
