 ...
 * <aies-chip variant="import" icon="truck">In transit</aies-chip>
 * <aies-chip variant="warning" [removable]="true" (removed)="clear()">Pending</aies-chip>
 * ```
 */
@Component({
  selector: 'aies-chip',
  ...
  template: `
    @if (icon(); as iconName) {
      <aies-icon [name]="iconName" [size]="iconSize()" class="shrink-0" />
    }
